...
#pragma omp for
                for (size_t j = 0; j < size; j++)
                {
                    int clusterIndex = observations[j].group;
                    local_clusters[clusterIndex].x += observations[j].x;
                    local_clusters[clusterIndex].y += observations[j].y;
...
#pragma omp critical
                {
                    for (int i = 0; i < k; i++)
                    {
                        clusters[i].x += local_clusters[i].x;
                        clusters[i].y += local_clusters[i].y;
...
/* PARALELIZAÇÃO: Leitura do número de threads desejado via argumento */
    ...
    if (argc > 1)
    {
        ...
        {
            fprintf(stderr,
                    "Invalid thread count '%s'. Using OpenMP default.\n",
                    argv[1]);
            requested_threads = 0;
        }
    }
